...
class Signaling {
  ...
  void onMessage(message) async {
    ...
    switch (mapData['type']) {
      ...
      case 'keep alive':
        {
          debugPrint('keep alive response!');
        }
        break;
      default:
        break;
  ...
  Future<void> connect() async {
    ...
    if (_turnCredential.isEmpty) {
      try {
        _turnCredential =
            (await getTurnCredential(_host, _port)) as Map<String, dynamic>;
        /*{
  ...
  Future<void> _cleanSessions() async {
    ...
    _sessions.forEach((key, sei) async {
      await sei.pc?.close();
      await sei.dc?.close();
    });
    _sessions.clear();
  ...
  void _closeSessionByPeerId(String peerId) {
    ...
    _sessions.removeWhere((String key, Session sei) {
      var ids = key.split('-');
      session = sei;
